...
async function fetchRepoInfo(owner, name, token) {
  const data = await fetchQuery(repoInfoGraphql, { owner, name }, token);
...
async function fetchCommitContributionsForRepo(owner, repo, since, authorId, token) {
  ...
  while (hasNextPage) {
    ...
    const data = await fetchQuery(query, variables, token);
...
async function fetchPRComments(owner, repo, prNumber, token) {
  ...
  let hasNextPage = true;
  while (hasNextPage) {
    const data = await fetchQuery(prCommentsGraphql, { owner, repo, prNumber, commentCursor }, token);
...
async function fetchPRReviewDetails(owner, repo, prNumber, token) {
  ...
  let hasNextPage = true;
  while (hasNextPage) {
    const data = await fetchQuery(prReviewDetailsGraphql, { owner, repo, prNumber, reviewCursor }, token);
...
async function fetchPRChangedFiles(owner, repo, prNumber, token) {
  const data = await fetchQuery(prChangedFilesGraphql, { owner, repo, prNumber }, token);
  return data.data?.repository?.pullRequest?.files?.nodes || [];
}

async function fetchPRTimeline(owner, repo, prNumber, token) {
  const data = await fetchQuery(prTimelineGraphql, { owner, repo, prNumber }, token);
