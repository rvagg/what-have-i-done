...
async function fetchQuery(query, variables, token, retryCount = 0) {
  ...
  try {
    ...
    if ((error.code === 'ECONNRESET' || error.code === 'ETIMEDOUT' || error.message.includes('fetch failed')) && retryCount < maxRetries) {
...
async function fetchRepoPRs(owner, repo, login, since, token, until = null) {
  let dateRange = `created:>=${since.toISOString().slice(0, 10)}`;
  if (until) {
    dateRange = `created:${since.toISOString().slice(0, 10)}..${until.toISOString().slice(0, 10)}`;
  }
...
async function fetchDirectRepoActivity(owner, repo, login, since, token, until = null) {
...
export async function fetchAdditionalReposActivity(repoList, login, since, token, until = null) {
  const results = await Promise.all(
    repoList.map(repoStr => {
      ...
      return fetchDirectRepoActivity(owner, repo, login, since, token, until);
    })
  );
...
export function generateActivityReport(activity, since, username, format = 'html', enrich = false, until = null) {
  ...
  // Track which repos have review data (repos from additional private repos won't have reviews)
  const repoStats = {};
  const reposWithReviewData = new Set(reviews.map(r => r.repository.nameWithOwner));
  // Repos from contributionsCollection have review data; additional repos queried directly do not
  const reposFromContributions = new Set([
    ...pullRequests.map(pr => pr.repository.nameWithOwner),
    ...issues.map(i => i.repository.nameWithOwner),
    ...commitsByRepo.map(c => c.repository.nameWithOwner),
    ...reviews.map(r => r.repository.nameWithOwner)
  ]);

  const getRepo = (name) => {
    if (!repoStats[name]) {
      repoStats[name] = { commits: 0, prsAuthored: 0, issuesCreated: 0, issuesCommented: 0, prsReviewedNonBot: 0, prsReviewedBot: 0, hasReviewData: false };
    }
  ...
  reviews
    ...
    .forEach((review) => {
      ...
      stats.hasReviewData = true;
      const prAuthor = review.pullRequest.author.login.toLowerCase();
      if (prAuthor === 'dependabot' || prAuthor.includes('bot')) {
  ...
  // Mark repos that came from contributionsCollection as having review data (even if 0 reviews)
  for (const repoName of reposWithReviewData) {
    getRepo(repoName).hasReviewData = true;
  }
  ...
  const repoSummaryData = Object.entries(repoStats)
    ...
    .map(([name, stats]) => {
      const noReviewData = !stats.hasReviewData && !reposFromContributions.has(name);
