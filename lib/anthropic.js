...
Make sure all HTML tags are properly closed and the content is well-structured.`;
    ...
    let currentLines = activityText.split('\n');
    ...
    while (inputTokens > maxInputTokens && compressionLevel < compressionLevels.length) {
      ...
      currentText = level.fn(currentLines);
    ...
    if (inputTokens > maxInputTokens) {
      ...
      requestBody = buildRequestBody(currentText, systemPrompt, model, maxTokens, isMultiUser, usernames, compressionNote);
      inputTokens = await countTokens(requestBody, apiKey);
      console.log(`Hard truncation applied: ${inputTokens} tokens (limit: ${maxInputTokens})`);

      // If still over, trim further
      if (inputTokens > maxInputTokens) {
        const retryRatio = maxInputTokens / inputTokens;
        currentText = currentText.slice(0, Math.floor(currentText.length * retryRatio * 0.90));
        requestBody = buildRequestBody(currentText, systemPrompt, model, maxTokens, isMultiUser, usernames, compressionNote);
        inputTokens = await countTokens(requestBody, apiKey);
        console.log(`Additional hard truncation applied: ${inputTokens} tokens (limit: ${maxInputTokens})`);
      }
    ...
    const compressionInfo = compressionLevel > 0 || hardTruncated
      ? {
          level: compressionLevel,
          dropped: [
            ...(compressionLevel > 0 ? compressionLevels[compressionLevel - 1].dropped : []),
            ...(hardTruncated ? ['Report was hard-truncated to fit'] : []),
          ],
        }
      : null;
